...
import { Slide, SlideType } from '../types/index.js';

export class SlideRenderer {
    /**
     * Render a slide to HTML element
     */
    public render(slide: Slide, index: number): HTMLElement {
     ...
     */
    private getSlideClasses(slide: Slide): string {
        const classes = ['slide'];
     ...
     */
    private applyBackground(element: HTMLElement, background: Slide['background']): void {
     ...
     */
    private renderContent(slide: Slide, index: number): string {
        switch (slide.type) {
            case 'title':
     ...
     * Render title slide
     */
    private renderTitleSlide(slide: Slide): string {
        return `
     ...
     */
    private renderSectionSlide(slide: Slide, index: number): string {
     ...
     */
    private renderContentSlide(slide: Slide): string {
        let contentHtml = '';
     ...
     */
    private renderComparisonSlide(slide: Slide): string {
        const comparison = slide.content.comparison;
        if (!comparison) return this.renderDefaultSlide(slide);
     ...
     * Render chart slide
     */
    private renderChartSlide(slide: Slide): string {
        const chart = slide.content.chart;
        if (!chart) return this.renderDefaultSlide(slide);
     ...
     * Render quote slide
     */
    private renderQuoteSlide(slide: Slide): string {
        const quote = slide.content.quote;
        if (!quote) return this.renderDefaultSlide(slide);
     ...
     */
    private renderConclusionSlide(slide: Slide): string {
        return `
            <div class="slide-content slide-content--left">
                <div class="slide-header">
                    <h2 class="slide-title">${slide.title}</h2>
     ...
     * Render thanks slide
     */
    private renderThanksSlide(slide: Slide): string {
        return `
            <div class="slide-content">
                <h1 class="slide-title">${slide.title}</h1>
                ${slide.subtitle ? `<p class="slide-subtitle">${slide.subtitle}</p>` : ''}
     ...
     * Render table
     */
    private renderTable(table: { headers: string[]; rows: string[][] }): string {
        return `
